...
### 7. Install Node.js (for PM2 and direct deployment option)
...
# Or if using SSH
git clone git@github.com:Michaelanand123singh/jewellery.git .
```
...
### PM2 Alternative (If Not Using Docker)
...
# Start with PM2
pm2 start npm --name "jewellery-app" -- start
...
1. **SSH Key Pair** - Generate SSH key for GitHub Actions to access your VPS
2. **GitHub Secrets** - Configure secrets in GitHub repository settings
3. **Deployment Scripts** - Scripts on server to handle deployments
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to staging server
        run: |
          ssh -p ${{ secrets.STAGING_SSH_PORT || 22 }} \
            ${{ secrets.STAGING_SSH_USER }}@${{ secrets.STAGING_HOST }} \
            'bash /root/deploy-staging.sh'

      - name: Deployment notification
        if: success()
        run: |
          echo "✅ Staging deployment completed successfully!"
          echo "🌐 Staging URL: https://staging.adorneluxejewels.com"
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Add server to known hosts
        run: |
          ssh-keyscan -p ${{ secrets.PRODUCTION_SSH_PORT || 22 }} ${{ secrets.PRODUCTION_HOST }} >> ~/.ssh/known_hosts

      - name: Deploy to production server
        run: |
          ssh -p ${{ secrets.PRODUCTION_SSH_PORT || 22 }} \
            ${{ secrets.PRODUCTION_SSH_USER }}@${{ secrets.PRODUCTION_HOST }} \
      ...
      - name: Deployment notification
        ...
        run: |
          ...
          echo "🌐 Production URL: https://adorneluxejewels.com"
...
You can add pre-deployment checks to the workflows:
...
- **GitHub Actions Tab**: View deployment status and logs
- **Server Logs**: Check deployment script logs
...
```bash
# On server
cd /var/www/production  # or /var/www/staging
...
4. **Deploy Production** - Only after staging is verified
5. **Set Up Monitoring** - Monitor logs and performance
6. **Configure Backups** - Ensure data safety
7. **Security Audit** - Review all security settings
